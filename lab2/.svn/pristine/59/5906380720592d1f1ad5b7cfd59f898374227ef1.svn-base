...
int hook_id = TIMER0_IRQ;  // hook_id = 0 (hook_id tem que ter valor entre 0 e 31, já que é
...
int (timer_subscribe_int)(uint8_t *bit_no) {
    ...
    if (sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &hook_id) != OK)  // Esta função muda o valor de hook_id
        return 1;

    *bit_no = hook_id;

    /*
    if (sys_irqenable(&hook_id) != OK)  // Ativa interrupções na IRQ line associadas ao hook_id;
